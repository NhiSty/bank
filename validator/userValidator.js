...
async function debitMoneyToAccount(user, accountId, money) {
	const prisma = new PrismaClient();
	...
	if (account.money - money < 0) {
		// add enought money to reach 1000 and return the rest
		const rest = money - account.money;
		account.money = 0;
		await prisma.account.update({ where: { id: accountId }, data: { money: account.money } });

		return { status: 200, body: { message: 'You have reach the limit of 0€', rest } };
	...
	return { status: 200, body: { message: `You withdraw money to your account, your new balance is ${account.money}` } };
...
module.exports = { DoNotHaveFiveAccount, addMoneyToAccount, debitMoneyToAccount };
